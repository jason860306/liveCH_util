...
public:
	...
	explicit CUriParser(const std::string& uri);

	/// Parses an URI from the given string. Throws a
	/// SyntaxException if the uri is not valid.
	explicit CUriParser(const char* uri);

	/// Creates an URI from its parts.
	CUriParser(const std::string& scheme, const std::string& pathEtc);

	/// Creates an URI from its parts.
	CUriParser(const std::string& scheme, const std::string& authority, const std::string& pathEtc);

	/// Creates an URI from its parts.
	CUriParser(const std::string& scheme, const std::string& authority, const std::string& path, const std::string& query);

	/// Creates an URI from its parts.
	CUriParser(const std::string& scheme, const std::string& authority, const std::string& path, const std::string& query, const std::string& fragment);
	
	/// Copy constructor. Creates an URI from another one.
	CUriParser(const CUriParser& uri);

	/// Creates an URI from a base URI and a relative URI, according to
	/// the algorithm in section 5.2 of RFC 3986.
	CUriParser(const CUriParser& baseURI, const std::string& relativeURI);

	~CUriParser();

	/// Assignment operator.
	CUriParser& operator = (const CUriParser& uri);

	/// Parses and assigns an URI from the given string. Throws a
	/// SyntaxException if the uri is not valid.
	CUriParser& operator = (const std::string& uri);

	/// Parses and assigns an URI from the given string. Throws a
	/// SyntaxException if the uri is not valid.
	CUriParser& operator = (const char* uri);

	/// Swaps the URI with another one.
	void swap(CUriParser& uri);
	...
	/// Resolves the given relative URI against the base URI.
	/// See section 5.2 of RFC 3986 for the algorithm used.
	void resolve(const CUriParser& relativeURI);
	...
	bool operator == (const CUriParser& uri) const;
...
protected:
	...
	///
	bool equals(const CUriParser& uri) const;
...
// inlines
//
inline const std::string& CUriParser::getScheme() const
{
	return _scheme;
}


inline const std::string& CUriParser::getUserInfo() const
{
	return _userInfo;
}


inline const std::string& CUriParser::getHost() const
{
	return _host;
}


inline const std::string& CUriParser::getPath() const
{
	return _path;
}


inline const std::string& CUriParser::getRawQuery() const
{
	return _query;
}


inline const std::string& CUriParser::getFragment() const
{
	return _fragment;
}


inline void swap(CUriParser& u1, CUriParser& u2)
{
	u1.swap(u2);
}
